...
class TieredImagenetLoaderCustom(data.Dataset):
    def __init__(self, root, partition='train'):
        super(TieredImagenetLoaderCustom, self).__init__()
        # set dataset information
        self.root = root
        self.partition = partition
        ...
        # set transformer
        if self.partition == 'train':
            self.transform = transforms.Compose([transforms.RandomCrop(224, padding=4),
                                                 lambda x: np.asarray(x),
                                                 transforms.ToTensor(),
                                                 normalize])
        else:  # 'val' or 'test' ,
    ...
    def load_dataset(self):
        # load data
        dataset_path = os.path.join(self.root, 'tiered_imagenet/compacted_datasets', 'tiered_imagenet_%s.pickle' % self.partition)
        with open(dataset_path, 'rb') as handle:
            data = pickle.load(handle)
